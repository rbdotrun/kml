    ...
    class Daytona
      ...
      def wait_for_snapshot(id, timeout: 600, interval: 5)
        ...
        loop do
          ...
          raise Kml::Error, "Snapshot build failed: #{state}" if %w[error failed].include?(state)
          raise Kml::Error, "Timeout waiting for snapshot #{id}" if Time.now > deadline
          sleep interval
        end
      end
      ...
      def wait_for_sandbox(id, timeout: 120, interval: 3)
        ...
        loop do
          ...
          raise Kml::Error, "Sandbox failed: #{state}" if %w[error failed].include?(state)
          raise Kml::Error, "Timeout waiting for sandbox #{id}" if Time.now > deadline
          sleep interval
        end
      end
      ...
      def upload_file(sandbox_id:, path:, content:)
        upload("toolbox/#{sandbox_id}/toolbox/files/upload", file_path: path, content:)
      end
      ...
      def stream_command_logs(sandbox_id:, session_id:, command_id:, &block)
        ...
        preview = get_signed_preview_url(sandbox_id:, port: 2280)
      ...
      def run_pty_command(sandbox_id:, command:, timeout: 600, cwd: nil, &block)
        ...
        create_pty_session(sandbox_id:, session_id: pty_id, cwd:)
      ...
      private
        ...
        def handle_response(response)
          ...
          raise Kml::Error, "[#{response.status}] #{error_msg}: #{body_info}"
        end
    end
  end
end
